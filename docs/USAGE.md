...
| `contribution_balance_alpha` (v26) | Strength for `contribution_balance`. `0` = no effect, `1` = full equalization before weights are applied. Since v27.5 the one-dial `AnimaArtistStyleBalance` node (compatible with upstream 26.x workflows) sets both keys at once, and the Probe Report suggests it when one artist's contribution verdict is "dominant" |
| `artist_q_reuse` (v27.5) | Experimental multi-artist speed-up: Q is projected once per step and reused for every artist K/V (numerically validated on first use). Default off — the fp16 kernel difference shifts same-seed renders (~17% of pixels in live A/B) and the fast path bypasses TeaCache-style attention patches |
